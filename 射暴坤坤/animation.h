...
public:
	...
	void add_frame(Atlas* atlas) {
		for (int i = 0; i < atlas->get_size(); i++) {
			...
			int width, heithg;
			SDL_QueryTexture(texture, nullptr, nullptr, &width, &heithg);

			SDL_Rect rect_src;
			rect_src.x = 0, rect_src.y = 0;
			rect_src.w = width, rect_src.h = heithg;
	...
	void on_render(const Camera& camera) const {
		...
		SDL_FRect rect_dst;
		rect_dst.x = pos.x - frame.rect_src.w / 2;
		rect_dst.y = pos.y - frame.rect_src.h / 2;
...
private:
	...
	std::vector<Frame> frame_list;		// 动画帧
